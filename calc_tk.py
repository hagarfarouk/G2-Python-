...
#####################################################
def clear():                            # clearing the feild
    global field_txt
    field_txt = " "
    field.delete("0", "end")
	
def press(value):						#storing the expersion
    global field_txt
    field_txt =  field_txt + str(value)
    field.delete("0","end")
    field.insert("0",field_txt)
	
def equal():								#giving the result
    global field_txt
...
press_equal =  Button(window_2, text="=", width=5, height=15, font=("Comic Sans MS", 10, "bold"), background='saddlebrown',
            bd='5',command=lambda :equal()).place(x=220, y=160)	
